...
namespace exp_bind
{
	...
	void next_impl(node& n, wrapper& rw)
	{
		...
		if constexpr (
			std::is_same_v<
			typename node::next_type::e_type, 
			wrapper>
			)
		{
	...
	void fill_from_tuple(node& n, _tuple& tp)
	{
		...
		if constexpr (node::has_next && 
			Index <= max_type_list_index<_tuple>::value)
		{
			auto rwp = ref_wrapper(std::get<Index>(tp));
			next_impl(n, rwp);
			function_forwarder<node, void, _tuple&> ff;
			ff.call = fill_from_tuple<
				typename forwarder<node>::type,
				Index + 1,
				_tuple>;
	...
	auto link_from_tuple(node& n,auto& _otp, _tuple<L...>& tp)
	{
		...
		// get first node type
		using first_node_type = typename tuple_iterator_types<
			join_tuple_type
		>::first_node_type;
	...
	{
		tuple_iterator<
			typename std::remove_reference<decltype(tp1)>::type
		> tp1_iter{ tp1 };
	...
	{
		_node _n(fv);
		(::push_back(_n, l, []<class Ty, class T>(T const& val) { 
			return std::make_shared<Ty>(val); }),
			...);
		return _n;
	}
	...
	auto join_list(_node1& _n1, _node2& _n2)
	{
		...
		using node_type = element_node<0, 
			join_list_type_impl,
			std::shared_ptr>;
	...
	{
		using _node_type = typename exp_apply<
			typename _node::element_type_list, 
			auto_ref_unwrapper
		>::type;
		using _node_impl = typename exp_rename<
			_node_type, 
			exp_shared_node
		>::type;
	...
	{
		using tuple_type_list = typename exp_apply<
			typename _node::element_type_list,
			auto_ref_unwrapper
		>::type;
		using tuple_type = typename exp_rename<
			tuple_type_list, 
			std::tuple
		>::type;
	...
	{
		using node_type_impl = typename exp_rename<
			typename exp_join_a_lot<TP, TPS...>::type, 
			exp_list>::type;
		using node_type_list = typename exp_apply<
			node_type_impl, ref_wrapper>::type;
		using node_type = element_node<
			0, 
			node_type_list, 
			std::shared_ptr>;
